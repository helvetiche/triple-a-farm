...
export const toastCRUD = {
  ...
  createError: (entity: string, error?: string) =>
    showErrorToast(`Failed to create ${entity}`, {
  ...
  loadError: (entity: string) =>
    showErrorToast(`Failed to load ${entity}`, {
  ...
  updateError: (entity: string, error?: string) =>
    showErrorToast(`Failed to update ${entity}`, {
  ...
  deleteError: (entity: string, error?: string) =>
    showErrorToast(`Failed to delete ${entity}`, {
      description: error || "Please try again or contact support.",
    }),

  // Rooster specific toasts
  roosterAdded: (name: string) => toastCRUD.createSuccess("Rooster", name),

  roosterUpdated: (name: string) => toastCRUD.updateSuccess("Rooster", name),

  roosterDeleted: (name: string) => toastCRUD.deleteSuccess("Rooster", name),

  roosterSold: (name: string, price: string) =>
    showSuccessToast(`Rooster "${name}" sold successfully`, {
      description: `Sale completed for ${price}.`,
    }),

  // Status changes
  statusChanged: (entity: string, name: string, status: string) =>
    showSuccessToast(`${entity} "${name}" status updated`, {
