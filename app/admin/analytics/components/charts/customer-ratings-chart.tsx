...
export function CustomerRatingsChart({
  title,
  description,
  data,
}: CustomerRatingsChartProps) {
  ...
  const ratingCounts = [5, 4, 3, 2, 1].map((stars) => ({
    ...
    percentage:
      data.length > 0
        ? (data.filter((item) => item.rating === stars).length / data.length) *
          100
        : 0,
  }));
      ...
      <CardContent style={{ borderRadius: 0 }}>
        <div className="space-y-6">
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-[#3d6c58]">
              {averageRating}
            </div>
            ...
            {ratingCounts.map((rating) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-24 bg-gray-200 h-2"
          ...
          <div className="border-t pt-4">
            <h4 className="text-sm font-medium text-[#1f3f2c] mb-2">
              Recent Activity
            </h4>
            <div className="text-xs text-gray-600">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-green-600">
                  {(
                    ((ratingCounts[0].count + ratingCounts[1].count) /
                      data.length) *
                    100
                  ).toFixed(0)}
                  %
